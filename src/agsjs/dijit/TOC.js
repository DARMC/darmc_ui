...
"dijit/popup",
 ...
 "dojo/_base/sniff"], function(
  ...
  var mySlider = new HorizontalSlider({
                    name: "slider",
                    value: 3,
                    minimum: 0,
                    maximum: 10,
...
/* 
  ...
  */
  ...
  var _TOCNode = declare([_Widget, _Templated],{
    ...
    '</span><span data-dojo-attach-point="pdNode" id="${serviceLayer.id}" data-dojo-attach-event="mouseover:_onMouseoverPD"></span>' +
    '<span data-dojo-attach-point="pdZoom" data-dojo-attach-event="onclick:_onClickPDZoom"></span></span></div>' +
    ...
    //console.log(this.data._subLayerInfos)
  ...
  // a layer inside a map service.
    ...
    if (serviceLayer._subLayerInfos) {// group layer
      ...
      if(serviceLayer.id == 30 || serviceLayer.id == 0 || serviceLayer.id == 76 || serviceLayer.id == 162){
        ...
        // remove this node if you want to have a check box at the group lavel
        this._noCheckNode = true;
      }
      ...
      domClass.add(this.pdNode, 'agsjsTOCServiceLayerPD');
      domClass.add(this.pdZoom, 'agsjsTOCServiceLayerPDZoom');
  ...
  _layerZoomHandler: function(response, io) {
    //var extent = new extent(response.extent);
    //map.setExtent(extent);
    console.log('test')
  },
  
  _onClickPDZoom: function(evt){
    ...
    ioScript.get({
      ...
      error: function(response, args){
        console.log("Error making JSONP request");
      }
    });
  ...
  _onMouseoverPD: function(evt){
    ...
    ioScript.get({
      ...
      load: function(response, args){
        ...
        //var zBtn = "<div dojoType='dijit/form/Button' load=console.log(foo); >Zoom to layer <img src='../css/images/zoom.png' width='13' height='13'></div>"
        //var zBtn = "<div dojoType='dijit.form.Button' onClick=map.setExtent(response.extent);>Zoom to layer <img src='../css/images/zoom.png' width='13' height='13'></div>"
        //myDialog.set("title", response.name);
        
        var zBtn = "<div id='slider'></div>"
        mySlider.style = "width:20px;"
        ...
        myDialog.set("content", response.copyrightText + zBtn);
        //mySlider.placeAt(zBtn);
        var pos = dojo.position(response.id.toString(), true);
        console.log(pos);
        dijit.popup.open({x: pos.x ,y: pos.y , popup: myDialog});
        //mySlider.placeAt(foo);
        //myDialog.set("content", response.copyrightText);
        //myDialog.show()
        //map.setExtent(foo);
